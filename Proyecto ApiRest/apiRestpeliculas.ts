...
app.get("/peliculas/:id", function(request, response){
    ...
    let respuesta=null
    if(id>=1 && id<=arrayMovs.length){
        respuesta = {error: false, codigo: 200, Movie: arrayMovs[id-1]};
    }else{
...
app.get("/peliculas", function(request, response){
    let respuesta = {error: false, codigo: 200, Array: arrayMovs};
    response.send(respuesta)
})
//Post 
...
app.put('/peliculas/:id', function(request,response){
    let id= request.params.id;
    arrayMovs[id-1].setTitle(request.body.title),
    arrayMovs[id-1].setReleaseYear(request.body.releaseYear),
    arrayMovs[id-1].setNationality(request.body.nationality), 
    arrayMovs[id-1].setActors(request.body.actors),
    arrayMovs[id-1].setDirector(request.body.director),
    arrayMovs[id-1].setWriter(request.body.writer), 
    arrayMovs[id-1].setLanguage(request.body.language), 
    arrayMovs[id-1].setPlatform(request.body.platform), 
    arrayMovs[id-1].setIsMCU(request.body.isMCU), 
    arrayMovs[id-1].setMainChName(request.body.mainCharacterName), 
    arrayMovs[id-1].setProducer(request.body.producer),  
    arrayMovs[id-1].setDistributor(request.body.distributor), 
    arrayMovs[id-1].setGenre(request.body.genre)
...
app.delete('/peliculas/:id', function(request, response){
    let id= request.params.id;
    pNueva=arrayMovs[id-1];
    arrayMovs.splice((id-1), 1);
    let respuesta = {error: false, codigo: 200, Mensaje: 'Pelicula Borrada', Movie: pNueva};
    response.send(respuesta)
});
//ERROR
